    ...
    >
      ...
      <td>
        <div className="flex items-center gap-2">
          ...
          {role === "admin" && (
            <>  {/* edit and delete */}
            <Link href={`/list/subjects/${item.id}`} className='flex items-center justify-center rounded-full bg-gray-300 w-8 h-8'> <FaEdit className='text-blue' /> </Link>
            <Link href={`/list/subjects/${item.id}`} className='flex items-center justify-center rounded-full bg-gray-300 w-8 h-8'> <FaTrash className='text-red-500' /> </Link>
          </>
          )}
        </div>
      </td>
